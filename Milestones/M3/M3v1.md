...
The pocess for Github Organization comes by directing the team to create feature branches and feed their changes pertanent to the feature. As pull requests were made, the team would requst a merge from the GitHub master. If a conflict had been made, the responsible party for that code would clear the conflict as to be sure their changes reflected their desired outcome. Files within the repository were thoroughly reviewed for organization in a manner that makes sense for oragnization and readability. This is to ensure project integrity and ease of building out further features. Each file commited was meticulously reviewed for errors or break changes. Any problematic merges were swiftly fixed by the code owner. Commit messages and comments to the code are mandatory for ease of review.
...
In addition to this, another framework that was a huge help was the EER diagram/table that was made. The diagram allowed us to ensure that the tables and databases that we're connecting to the backend are working correctly with their unique keys. With the tables and keys established in the EER, the relationships have helped greatly when working with the features and design of our product. Furthermore, the frameworks are now organized in a way that allows us to work efficiently.
